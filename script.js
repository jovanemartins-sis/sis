...
// =========================================================
// LÓGICA DE FILTRAGEM DE PEDIDOS POR INTEGRAÇÃO ATIVA
// =========================================================
...
function setupCadastroEmpresasPage() {
    ...
    if (salvarBtn) {
        salvarBtn.addEventListener('click', () => {
            alert("Cadastro de empresa salvo com sucesso! (Simulação)");
            // Opcional: Voltar para a página de empresas
            loadPage('empresas');
        });
    }
}
...
function loadPage(pageName) {
    ...
    // Lógica para marcar o item do menu como ativo e abrir o menu pai
    document.querySelectorAll('#sidebar li').forEach(li => li.classList.remove('active'));
    ...
    if (activeLink) {
        ...
        if (parentMenu) {
            ...
            // Remove a classe 'active' do link pai para não ter dois ativos visuais
            parentMenu.querySelector('a[data-page]').closest('li').classList.remove('active');
        }
    }
    ...
    if (pageName === 'pedidos') {
        ...
        const allIntegrations = getIntegracoes();
        // Filtragem mockada para mostrar integrações "novas"
